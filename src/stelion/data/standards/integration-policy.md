...
Guidelines for integrating ecosystem packages into consumer projects. Governs
the choice of integration mechanism, version pinning strategy, and optional
dependency patterns.
...
### 1.3 Canonical Mechanism per Library

Project-specific mechanism assignments are declared in `stelion.yml` under
`integrations.canonical_mechanisms`. See the workspace manifest for the current
assignments.

### 1.4 Submodule Directory Conventions

- `vendor/` -- standalone tools invoked by the consumer
- `common/` -- shared content included into the consumer's source
...
### 3.1 When to Declare Optional Dependencies
...
if TYPE_CHECKING:
    from external_lib.io import Saver

class IOHandler:
    """Bridge between the consumer and an external library's I/O."""

    def save(self, data: DataStructure, path: Path) -> None:
        from external_lib.io import get_saver  # lazy runtime import
        saver = get_saver(path.suffix)
...
Project-specific reference implementations are listed in `stelion.yml` under
`integrations.reference_implementations`. See the workspace manifest for the
current references.
...
```yaml
pip:
  - -e /absolute/path/to/package[dev]
  - -e ../../relative/path/to/package[dev]
```

Absolute paths are acceptable for single-developer workflows. Switch to relative
paths from the consumer's root if portability is required.
