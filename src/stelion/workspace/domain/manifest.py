...
class TemplateConfig:
    ...
    def __post_init__(self) -> None:
        object.__setattr__(self, "delimiters", _to_str_tuple(self.delimiters, "template.delimiters"))
        if len(self.delimiters) != 2:
            raise ManifestValidationError("Section 'template.delimiters' must contain exactly two strings.")
        object.__setattr__(
            self,
            "exclude_patterns",
            _to_str_tuple(self.exclude_patterns, "template.exclude_patterns"),
        )
        object.__setattr__(self, "renames", MappingProxyType(_to_str_map(self.renames, "template.renames")))
...
class VSCodeConfig:
    ...
    def __post_init__(self) -> None:
        object.__setattr__(
            self,
            "settings_overrides",
            MappingProxyType(dict(_to_mapping(self.settings_overrides, "vscode.settings_overrides"))),
        )
        object.__setattr__(
            self,
            "extensions_overrides",
            _to_str_tuple(self.extensions_overrides, "vscode.extensions_overrides"),
        )
...
class WorkspaceManifest:
    ...
    def __post_init__(self) -> None:
        object.__setattr__(self, "names_in_use", MappingProxyType(_to_str_map(self.names_in_use, "names_in_use")))

    def to_dict(self) -> dict[str, Any]:
        """Serialize the manifest model into YAML-safe primitives."""
        return manifest_to_dict(self)

    @classmethod
    def from_dict(cls, raw: Mapping[str, Any], manifest_dir: Path) -> "WorkspaceManifest":
        ...
        return cls(
            ...
            template=TemplateConfig(
                ...
                delimiters=_coalesce_tuple(template_raw.get("delimiters"), defaults.template.delimiters, "template.delimiters"),
                ...
                renames=_coalesce_str_map(template_raw.get("renames"), dict(defaults.template.renames), "template.renames"),
            ),
            defaults=EcosystemDefaults(
...
def manifest_to_dict(manifest: WorkspaceManifest) -> dict[str, Any]:
    ...
    return {
        ...
        "dependencies": {
            ...
            "extra_scan_dirs": list(manifest.dependencies.extra_scan_dirs),
        },
...
def _parse_dependencies_config(raw: Mapping[str, Any]) -> DependenciesConfig:
    ...
    return DependenciesConfig(
        ...
        extra_scan_dirs=_coalesce_tuple(raw.get("extra_scan_dirs"), (), "dependencies.extra_scan_dirs"),
    )
...
def _coalesce_mapping(value: Any, default: Mapping[str, Any], label: str) -> Mapping[str, Any]:
    if value is None:
        return dict(default)
